...
float readACVoltage() {
  ...
  for (int i = 0; i < samples; i++) {
    ...
    // Shift center to 0V (ADC 2048 is approx 1.65V)
  ...
  // Apply ZMPT calibration factor
  return vrms * VOLTAGE_CAL * 100.0;
}
...
void setup() {
  ...
  analogSetAttenuation(ADC_11db); // 0-3.3V scaling for ADC
  ...
  // WiFi AP
  WiFi.softAP("EarthingGuard", "12345678"); 
  ...
  display.clearDisplay();
  display.setCursor(0,0);
  display.println("EarthingGuard WiFi");
  display.println(IP);
  display.display();
  delay(2000);
  ...
  server.on("/script.js", HTTP_GET, []() { server.send(200, "application/javascript", script_js); });
...
void loop() {
  ...
  // --- 1. Soil Moisture (Capacitive) ---
  digitalWrite(SOIL_POWER, HIGH);
  delay(10);
  int soilValue = analogRead(SOIL_PIN);
  digitalWrite(SOIL_POWER, LOW); // Prevent corrosion
  
  soilPercent = map(soilValue, 4095, 0, 0, 100);
  soilPercent = constrain(soilPercent, 0, 100);
  
  // --- 2. AC Voltage (ZMPT) ---
  voltageAC = readACVoltage();
  
  // --- 3. Safety Logic ---
  // "AC >= 2.55 && AC <= 2.61"
  soilGood = (soilPercent >= 25);
  ...
  if (!systemSafe) {
    ...
    // Non-blocking approach for web server performance might be better,
    // but sticking to requested simple logic with small delay is okay.
    // The BeepAlert function has delays, which pauses the webserver.
    // We'll use a millis check here to avoid freezing web interface.
  ...
  // --- 5. OLED Updates ---
  // (Reduced update rate to stop flicker and save CPU)
  ...
  if(millis() - lastOled > 500) {
    ...
    display.print("Volt: "); display.print(voltageAC, 2); display.println("V");
